...
class PolicyController extends AdminController {
    ...
    public function index(){
        ...
        $list = $this->lists('policy', $map,'id desc');
    ...
    public function setStatus($model=''){
      $status = I('get.status');
      $status = $status == 1 ? 0 : 1;
      $this->editRow('policy', array('status'=>$status), array('id'=>I('get.id')));
    }
    ...
    public function edit(){
        ...
        // 获取详细数据
        $data  =M('policy')->where(['id'=>$id])->find();
    ...
    public function update(){
        ...
        if($data['id']){
            $Sqldata['id'] = $data['id'];
            $policy->data($Sqldata)->save();
            $policy_id = $data['id'] ;
        }else{
            $policy_id = $policy->data($Sqldata)->add();
        }
       if(empty($policy_id)) {
