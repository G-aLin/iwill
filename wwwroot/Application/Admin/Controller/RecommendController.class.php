...
class RecommendController extends AdminController {
    ...
    public function index(){
        ...
        $list = $this->lists('recommend', $map,'id desc');
        $pictureM = M('picture') ;
    ...
    public function setStatus($model=''){
      $status = I('get.status');
      $status = $status == 1 ? 0 : 1;
      $this->editRow('recommend', array('status'=>$status), array('id'=>I('get.id')));
    }
 ...
 // var_dump($item_category) ; exit;
        ...
        $this->assign('item_category', $item_category);
        $this->display();
    }
    ...
    public function edit(){
        ...
        // 获取详细数据
        $data  =M('recommend')->where(['id'=>$id])->find();
 ...
 // var_dump($item_category) ; exit;
    ...
    public function update(){
        ...
        if($data['id']){
            $Sqldata['id'] = $data['id'];
            $recommend->data($Sqldata)->save();
            $recommend_id = $data['id'] ;
        }else{
            $recommend_id = $recommend->data($Sqldata)->add();
        }
