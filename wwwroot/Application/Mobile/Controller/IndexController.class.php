...
class IndexController extends HomeController {
    ...
    public function index(){
        ...
        $banner_list =M('banner b')->join('onethink_picture p ON p.id = b.picture_id')->where(['b.type'=>5,'b.status'=>1])->field('b.url,p.path')->order('b.level desc,b.id desc')->select();
        $this->assign('banner_list',$banner_list);//列表

        $rotation_list  =M('recommend b')->join('onethink_picture p ON p.id = b.picture_id')->where(['b.type'=>4,'b.status'=>1])->field('b.value,p.path')->order('b.level desc,b.id desc')->select();
